...
const books = Object.fromEntries(["oota"].map(id => [id, {
...
const summaries = [];
const preservedLiteralContexts = new Set([
	"data/8/entries/13/entries/1/entries/0", "data/8/entries/13/entries/2/entries/0",
	"data/8/entries/13/entries/3/entries/0", "data/8/entries/13/entries/4/entries/0",
	"data/11/entries/5/entries/3/entries/0", "data/12/entries/3/entries/11/entries/4/entries/0",
	"data/13/entries/8/entries/3", "data/13/entries/8/entries/7/entries/0",
	...[1, 2, 3, 4, 5, 6].map(ix => `data/14/entries/9/entries/4/entries/7/entries/2/entries/${ix}`),
	"data/14/entries/13/entries/4/entries/2",
]);
const walk = (en, zh, context, count) => {
	if (typeof en === "string") {
		...
		const sourceContext = context.replace(/^oota/u, "data");
		if (!sourceContext.startsWith("data/22/") && !preservedLiteralContexts.has(sourceContext)) {
			const visible = Renderer.stripTags(zh).replace(/\b(?:NPC|DM|DC|AC|HP|XP|PDF)\b/gu, "");
...
for (const [id, book] of Object.entries(books)) {
	...
	assert.equal(view.dataDocument.data.length, 23);
	...
	const terms = ["幽暗地域", "Underdark", "Demogorgon"];
	...
	const report = read(`translation/zh-TW/adventures/oota/${id}-import-review.json`);
	...
	for (const item of report.untranslatedVisibleStrings) assert.ok(item.context.startsWith("data/22/") || item.english === "NPC", `Untranslated prose: ${id}/${item.context}`);
	summaries.push(count);
}

// Regressions for source defects and mechanics found during OotA review.
const at = context => context.split("/").reduce((node, key) => node[key], books.oota.localized);
assert.match(at("data/14/entries/4/entries/3/entries/4/entries/0"), /\{@dice 1d4\}−1/u, "Surviving drow count lost −1");
assert.match(at("data/3/entries/15/entries/3/entries/2/entries/0"), /微型.*小型.*中型/u, "Narrow-tunnel size rules changed");
assert.match(at("data/3/entries/15/entries/7/entries/14/entries/4"), /微型/u, "Tiny cover recipients mistranslated");
assert.match(at("data/3/entries/15/entries/7/entries/5/entries/4"), /半身掩護/u, "Half cover became generic cover");
for (const [ix, keyword] of [[3, "圖書館"], [5, "冥想"]]) {
	const text = at(`data/11/entries/3/entries/1/entries/0/items/${ix}`);
	assert.ok(text.includes(keyword), "Tower-floor description omitted");
	assert.match(text, /層/u, "Tower level became spell level");
}
assert.match(at("data/7/entries/4/entries/20/entries/3"), /無底深淵.*幽暗地域/u, "Demon origin and current location conflated");
assert.match(at("data/11/entries/5/entries/3/entries/0"), /\{@i ku'lam\}.*\{@i ilkalik\}/u, "Command words changed");
assert.match(at("data/14/entries/9/entries/4/entries/7/entries/2/entries/4"), /\{@i Lolth tlu malla\}/u, "Literal Drow phrase changed");
assert.doesNotMatch(JSON.stringify(books.oota.localized.data), /虛構的材料|這裡暫按原詞|未提供具體翻譯|布靈登石城城|鼠人人|灰矮人爾|灰矮人岩岩石衛隊/u);
...
const allBooks = {...books, cos, ...Object.fromEntries(["wdh", "hotdq", "rot", "vnotee", "veor"].map(id => [id, {canonical: read(`data/adventure/adventure-${id}.json`), localized: read(`data/zh-TW/adventures/adventure-${id}.json`), index: indexes.find(it => it.id.toLowerCase() === id)}]))};
...
DataUtil.loadJSON = async url => allBooks[url.match(/adventure-([^.]+)\.json$/)?.[1]]?.canonical || books.oota.canonical;
...
try {
	for (const id of ["oota", "vnotee", "veor", "wdh", "cos", "hotdq", "rot", "oota"]) {
	...
	assert.deepEqual(fetches, ["oota", "vnotee", "veor", "wdh", "cos", "hotdq", "rot"], "Per-book cache failed");
	await BookUtil._pLoadChapter({...books.oota.index, id: "LMoP", name: "Lost Mine of Phandelver"}, "lmop", ["0"], null, {});
	assert.equal(BookUtil.referenceI18n, null);
	assert.equal(fetches.length, 7);
	assert.strictEqual(rendered.data, books.oota.canonical.data);
} finally {
