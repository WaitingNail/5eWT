...
繁體中文覆蓋層涵蓋 Out of the Abyss（OotA）的17章、4個附錄、後記及製作名單，共23個區塊。初版於 `codex/campaign-translations` 完成；2026-09-18 依使用者要求合併至 `main` 並發布，後續依核准譯名修訂。
...
- `site-name-corrections.json`：使用者核准的 OotA NPC 名稱，重建時同步怪物資料與圖片資料的顯示名稱，保留英文識別。
...
以下使用者指定覆蓋舊譯名，於術語表記為 `user-approved`、`lock=true`，並同步正文、標題、NPC 連結及短稱。

| 原文 | 核准譯名 |
| --- | --- |
| Velkynvelve | 隱刃哨所 |
| Buppido | 布比多 |
| Eldeth Feldrun | 埃德斯．費爾德倫 |
| Jimjar | 吉姆加爾 |
| Ront | 倫特 |
| Shuushar the Awakened | 覺醒者蘇沙爾 |
| Stool | 史都爾 |
| Topsy and Turvy | 托普西和托芮 |
| Shoor Vandree | 肖爾．汎德里 |

短稱依完整姓名採「埃德斯」「蘇沙爾」「肖爾」；雙人分開出現時為「托普西」「托芮」。
